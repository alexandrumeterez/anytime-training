...
from olmo.optim import BoltOnWarmupScheduler, LinearDecay, LinearWithWarmup
...
def test_linear_decay_checkpoint_preserves_cooldown_start():
    ...
    assert scheduler.get_lr(1.0, 900, 1000) == 1.0
    expected = scheduler.get_lr(1.0, 950, 1000)

    restored = LinearDecay(
        grad_clip_warmup_steps=None,
        grad_clip_warmup_factor=None,
        warmup_steps=0,
        warmup_min_lr=None,
    ...
    assert restored.get_lr(1.0, 950, 1000) == pytest.approx(expected)
