...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.wandb is not None and (get_global_rank() == 0 or not cfg.wandb.rank_zero_only):
        ...
        wandb.init(
            ...
            resume='must' if cfg.wandb.run_id is not None else 'allow'
        )
    ...
    if cfg.model.use_ema:
        evaluators = [build_evaluators(cfg, device, cfg.model.ema_decay[i]) for i in range(len(cfg.model.ema_decay))]
    else:
    ...
    ema_model = None
    if cfg.model.use_ema:
        emas_to_maintain = len(cfg.model.ema_decay)
        ema_model = [OLMo(cfg.model) for _ in range(emas_to_maintain)]
        if cfg.model.init_device != "meta":
            # EMA shadows must begin at the same point as the trainable model.
            # Independent construction consumes new RNG state and otherwise
            # gives every shadow a different initialization.
            model_state = olmo_model.state_dict()
            for shadow_model in ema_model:
                shadow_model.load_state_dict(model_state)
    ...
    if cfg.distributed_strategy == DistributedStrategy.ddp:
        ...
        if cfg.model.use_ema:
            ema_model = [DDP(ema_model[i].to(device), find_unused_parameters=cfg.ddp.find_unused_params) for i in range(len(ema_model))]
        ...
        if cfg.model.use_ema:
            ema_model = [
                SingleAccelerator(ema_model[i].to(device)) for i in range(len(ema_model))
            ]
    ...
    with Trainer(
        ...
        ema_model=ema_model if cfg.model.use_ema else None,
    ) as trainer:
        ...
        if cfg.load_path is not None:
            ...
            if cfg.model.use_ema:
                for i in range(len(trainer.ema_parameters)):
                    if cfg.model.ema_decay[i] == 0.0:
                        trainer.ema_parameters[i].shadow.module.load_state_dict(torch.load(os.path.join(cfg.load_path, 'model.pt')))
                    else:
                        trainer.ema_parameters[i].shadow.module.load_state_dict(torch.load(os.path.join(cfg.load_path + f"-ema-{cfg.model.ema_decay[i]}", 'model.pt')))
