...
class Scheduler(metaclass=ABCMeta):
    ...
    def state_dict(self) -> Dict[str, Any]:
        """Return mutable scheduler state that must survive a checkpoint."""
        return {}

    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        """Restore mutable scheduler state from a checkpoint."""
        del state_dict
...
class LinearDecay(Scheduler):
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        t_start = state_dict.get("t_start")
        self.t_start = None if t_start is None else int(t_start)
    ...
    def get_lr(self, initial_lr: float, step: int, max_steps: int) -> float:
        ...
        if step >= max_steps:
            ...
            return initial_lr * (1 - (1 - self.alpha_f) * (step - self.t_start) / (max_steps - self.t_start))
...
class InvSqrtWithWarmup(Scheduler):
    ...
    def get_lr(self, initial_lr: float, step: int, max_steps: int) -> float:
        if step < self.warmup_steps:
            return self._linear_warmup(initial_lr, step, self.warmup_steps)
        del max_steps
        return initial_lr * sqrt(self.offset / (step - self.warmup_steps + self.offset))
...
def build_scheduler(cfg: TrainConfig, sched_cfg: Optional[SchedulerConfig] = None) -> Scheduler:
    ...
    elif sched_cfg.name == SchedulerType.linear_decay:
        return LinearDecay(
            grad_clip_warmup_steps=(
                None if sched_cfg.grad_clip_warmup_steps is None else int(sched_cfg.grad_clip_warmup_steps)
            ),
