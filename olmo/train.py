...
class EMA(nn.Module):
    ...
    - decay ∈ [0,1). Higher = slower updates, more smoothing.
    - Use `ema.copy_to(model)` to load EMA weights into a model for eval.
    """
    def __init__(self, shadow: nn.Module, gamma: float = 0.999):
        super().__init__()
        self.shadow = shadow
        for p in self.shadow.parameters():
            p.requires_grad_(False)
        self.gamma = gamma
        self.ema_decay = (0.5) ** (self.gamma)
        self.shadow.eval()
    ...
    def update(self, model: nn.Module, step: int):
        if self.gamma == 0.0:
            ...
            self.ema_decay = (0.5) ** (self.gamma / (step + 1))
        for s_param, m_param in zip(self.shadow.parameters(), model.parameters()):
            s_param.mul_(self.ema_decay).add_(m_param, alpha=1.0 - self.ema_decay)
        # ema_t = ema_decay * ema_{t-1} + (1 - ema_decay) w_t = 
...
class Trainer:
    ...
    device: torch.device
    evaluators: List[Evaluator] | List[List[Evaluator]]
    ...
    ema_parameters: Optional[List[EMA]] = None
    ema_model: Optional[List[OLMo]] = None
    _optimizer_group_settings: List[Dict[str, Any]] = field(
        init=False, repr=False, default_factory=list
    )

    def __post_init__(self):
        # Optimizer state dicts include hyperparameters as well as moments. Keep
        # the values requested by the current config so a checkpoint fork may
        # intentionally change LR, betas, epsilon, or weight decay.
        configurable_fields = ("lr", "initial_lr", "betas", "eps", "weight_decay")
        self._optimizer_group_settings = [
            {key: group[key] for key in configurable_fields if key in group}
            for group in self.optim.param_groups
        ]
        self.ema_parameters = None
        if self.cfg.model.use_ema:
            assert self.ema_model is not None
            self.ema_parameters = [EMA(self.ema_model[i], gamma=self.cfg.model.ema_decay[i]) for i in range(len(self.ema_model))]
    ...
    def trainer_state_dict(self) -> Dict[str, Any]:
        return {
            ...
            "scheduler": {
                "class": type(self.scheduler).__qualname__,
    ...
    def load_trainer_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        # Restore stateful schedulers only when the checkpoint was created with
        # the same scheduler type. This preserves LinearDecay.t_start on a
        # mid-cooldown resume while still allowing a constant checkpoint to
        # branch into a fresh WSD cooldown.
        scheduler_state = state_dict.get("scheduler")
        if (
            scheduler_state is not None
            and scheduler_state.get("class") == type(self.scheduler).__qualname__
        ):
            self.scheduler.load_state_dict(scheduler_state.get("state", {}))

        # Reset optimizer hyperparameters to the current config, not the
        # checkpoint. Optimizer moments remain restored.
        log.info("Resetting optimizer hyperparameters...")
        new_learning_rate = self.scheduler.get_lr(
            self.cfg.optimizer.learning_rate, self.scheduler_current, self.scheduler_max
        )
        if len(self.optim.param_groups) != len(self._optimizer_group_settings):
            raise OLMoConfigurationError(
                "Optimizer parameter-group count changed after loading the checkpoint"
            )
        for group, configured in zip(self.optim.param_groups, self._optimizer_group_settings):
    ...
    def _save_checkpoint(
        ...
        if self.ema_parameters is not None:
            ema_values = self.cfg.model.ema_decay
            present_0 = False
            if ema_values[0] == 0.0:
                ema_values = ema_values[1:]
                present_0 = True
            for i, ema in enumerate(self.ema_parameters):
                if i != 0 or (i==0 and not present_0):
                    cur_path = Path(self.cfg.save_folder) / f"step{self.global_step}{suffix}-ema-{self.cfg.model.ema_decay[i]}"
                    try:
                        checkpointer.save_checkpoint(
                            cur_path,
                            ema.shadow,
                        ...
                        raise OLMoConfigurationError(
                            f"Had trouble saving EMA checkpoint for step {self.global_step}, use --save_overwrite to overwrite it"
                        )
        ...
        if link_latest:
            ...
            for decay in self.cfg.model.ema_decay:
                if decay == 0.0:
                    continue
                full_ema_path = Path(self.cfg.save_folder) / f"step{self.global_step}{suffix}-ema-{decay}"
                latest_ema_path = Path(self.cfg.save_folder) / f"latest{suffix}-ema-{decay}"
                latest_ema_path.unlink(missing_ok=True)
                try:
                    latest_ema_path.symlink_to(full_ema_path.name, target_is_directory=True)
                except FileExistsError:
                    # Same as above, caught when another (file-system) local rank 0 has already made the 'latest' symlink.
                    # This can happen when nodes are saving to a common NFS drive but otherwise have distinct
                    # file-systems.
                    if latest_ema_path.resolve().name != full_ema_path.name:
                        raise
    ...
    def model_forward(
        self, batch: Dict[str, Any], loss_reduction: str = "mean", compute_z_loss: bool = False, eval_index: int = 0
        ...
        elif not self.dist_model.training and self.cfg.model.use_ema:
            assert self.ema_parameters is not None
            logits = self.ema_parameters[eval_index].shadow(
                input_ids=batch["input_ids"],
                attention_mask=batch.get("attention_mask"),
                attention_bias=batch.get("attention_bias"),
                doc_lens=batch.get("doc_lens"),
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        if self.cfg.model.use_ema:
            assert self.ema_parameters is not None
            for i in range(len(self.ema_parameters)):
                self.ema_parameters[i].update(self.dist_model, self.global_step)
    ...
    def eval_batch(self, batch: Dict[str, Any], eval_index: int = 0) -> Tuple[torch.Tensor, torch.Tensor]:
        with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
            ce_loss, _, logits = self.model_forward(batch, loss_reduction="none", eval_index=eval_index)
        return ce_loss.mean(dim=-1), logits

    def eval_step(self, batch: Dict[str, Any], evaluator: Evaluator, eval_index: int = 0) -> None:
        # Move tensors to the right device.
        batch = move_to_device(batch, self.device)

        # Run forward pass.
        with torch.no_grad():  # NOTE: 'torch.inference_mode()' doesn't work with 'torch.compile()'.
            ce_loss, logits = self.eval_batch(batch, eval_index=eval_index)
    ...
    def should_eval_this_step(self) -> bool:
        ...
        if self.cfg.eval_interval is not None:
            ...
            if type(self.cfg.max_duration) == int:
                ...
                max_duration = int(float(self.cfg.max_duration[:-1]))
            logspace = np.logspace(-2.1, 0, self.cfg.eval_count_log_scale) * max_duration
            
            log_steps = [int(n / 100) * 100 for n in logspace]
            
            extra_log_steps = self.cfg.custom_checkpoint_saving_list
            for ls in extra_log_steps:
                if ls not in log_steps:
                    log_steps.append(ls-1)
                    log_steps.append(ls)
    ...
    def should_save_unsharded_this_step(self) -> bool:
        if len(self.cfg.custom_checkpoint_saving_list) > 0:
            if (self.global_step + 1) in self.cfg.custom_checkpoint_saving_list:
                return True
            else:
                return False
        ...
        if self.cfg.save_count_log_scale_unsharded is not None:
            if type(self.cfg.max_duration) == int:
                ...
                max_duration = int(float(self.cfg.max_duration[:-1]))
            logspace = np.logspace(-2.1, 0, self.cfg.save_count_log_scale_unsharded) * max_duration
            log_steps = [int(n / 100) * 100 for n in logspace]
            return self.global_step in log_steps
        else:
            return False

    def eval(self, eval_index: int = 0) -> Dict[str, Any]:
        # Zero gradients and set model to 'eval' mode.
        self.optim.zero_grad(set_to_none=True)
        self.dist_model.eval()

        eval_metrics = {}
        eval_list = self.evaluators[eval_index] if self.cfg.model.use_ema else self.evaluators
        for evaluator in eval_list:
            ...
            for eval_step, eval_batch in enumerate(eval_batches):
                self.eval_step(eval_batch, evaluator, eval_index=eval_index)
            ...
            for epoch in range(self.cfg.epochs):
                ...
                for batch in self.train_loader:
                    ...
                    if self.cfg.model.use_ema:
                        assert self.ema_parameters is not None
                        for i in range(len(self.ema_parameters)):
                            metrics.update({f'ema_decay_{self.ema_parameters[i].gamma}': self.ema_parameters[i].ema_decay})
                    ...
                    if not cancel_initiated and self.should_eval_this_step():
                        if self.cfg.model.use_ema:
                            for eval_index in range(len(self.evaluators)):
                                eval_metrics = self.eval(eval_index=eval_index)
                                if wandb.run is not None:
                                    wandb.log(eval_metrics, step=self.global_step)
