        ...
        if self.type == EvaluatorType.downstream:
            ...
            for label in sorted(metrics.keys()):
                ...
                if loss.isnan().item():
                    ...
                    out[f"eval/{label}/{self.label}/CrossEntropyLoss"] = loss.item()
                    # Keep the evaluator label in the perplexity key as well.  The
                    # previous generic key was overwritten when several EMA
                    # evaluators were logged in the same evaluation pass.
                    perplexity = torch.exp(loss).item()
                    out[f"eval/{label}/{self.label}/Perplexity"] = perplexity
                    # Preserve the historical convenience alias only when
                    # there is a single, non-EMA evaluator.
                    if "_ema_" not in self.label:
                        out[f"eval/{label}/Perplexity"] = perplexity
            return out
        else:
