...
def build_evaluator(
    train_config: TrainConfig, eval_config: EvaluatorConfig, tokenizer: Tokenizer, device: torch.device, eval_decay_val: Optional[float] = None,
) -> Evaluator:
    ...
    if eval_config.type == EvaluatorType.downstream:
        ...
        return Evaluator(
            label=eval_config.label if eval_decay_val is None else f"{eval_config.label}_ema_{eval_decay_val}",
...
def build_evaluators(cfg: TrainConfig, device: torch.device, eval_decay_val: Optional[float] = None,) -> List[Evaluator]:
    evaluators = []
    tokenizer = Tokenizer.from_train_config(cfg)
    for eval_cfg in cfg.evaluators:
        evaluators.append(build_evaluator(cfg, eval_cfg, tokenizer, device, eval_decay_val=eval_decay_val))
    return evaluators
